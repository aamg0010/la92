...
import { useFinancialMetrics, useServiceBreakdown, useDoctorProductivity, getPeriodDates, getPreviousPeriodDates } from "@/hooks/useFinances";
...
const MetricCard = ({ title, value, change, changeLabel, icon: Icon, trend, isLoading }: MetricCardProps) => (
...
const Finanzas = () => {
  const period = getPeriodDates("month");
  const prevPeriod = getPreviousPeriodDates(period.startDate, period.endDate);
      ...
      <div className="p-6 lg:p-8 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Análisis financiero del consultorio
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard
            ...
            value={String(metrics?.patientsAttended ?? 0)}
            change={metrics?.patientsChange ?? 0}
            changeLabel="vs mes anterior"
            icon={Users}
            trend={metrics?.patientsChange && metrics.patientsChange > 0 ? "up" : metrics?.patientsChange && metrics.patientsChange < 0 ? "down" : "neutral"}
            isLoading={loadingMetrics}
          />
          <MetricCard
            title="Procedimientos"
            value={String(metrics?.totalProcedures ?? 0)}
            change={metrics?.proceduresChange ?? 0}
            changeLabel="vs mes anterior"
            icon={Calendar}
            trend={metrics?.proceduresChange && metrics.proceduresChange > 0 ? "up" : metrics?.proceduresChange && metrics.proceduresChange < 0 ? "down" : "neutral"}
            isLoading={loadingMetrics}
          />
          <MetricCard
            ...
            changeLabel="vs mes anterior"
            icon={Target}
            trend={metrics?.ticketChange && metrics.ticketChange > 0 ? "up" : metrics?.ticketChange && metrics.ticketChange < 0 ? "down" : "neutral"}
            isLoading={loadingMetrics}
          />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="card-elevated p-6">
            ...
            {loadingServices ? (
              ...
              <p className="text-center py-12 text-muted-foreground">No hay datos de servicios</p>
            ) : (
              <div className="space-y-4">
                {serviceBreakdown.slice(0, 5).map((service, index) => {
                  const percentage = totalServiceRevenue > 0 
                    ? Math.round((service.revenue / totalServiceRevenue) * 100) 
                    : 0;
                  const colors = ["bg-primary", "bg-accent", "bg-success", "bg-warning", "bg-muted-foreground"];
                  return (
        ...
        <div className="card-elevated p-6">
          ...
          {loadingProductivity ? (
            ...
            <p className="text-center py-12 text-muted-foreground">No hay datos de productividad</p>
          ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="card-elevated p-6 text-center">
            ...
            <p className="text-2xl font-display font-bold text-foreground">
              {metrics?.patientsAttended ? Math.min(100, Math.round((metrics.patientsAttended / 30) * 100)) : 0}%
            </p>
